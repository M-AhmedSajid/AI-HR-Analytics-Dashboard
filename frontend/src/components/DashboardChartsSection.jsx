...
export default function DashboardChartsSection({ employees }) {
  ...
  const slicedEmps = employees.slice(0, 5);
  const salaryPerformance = slicedEmps.map((employee) => {
    const parts = employee.name.split(" ");
    const firstName = parts[0].endsWith(".") && parts[1] ? parts[1] : parts[0];
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm dark:bg-[#0b0b0b] dark:border dark:border-zinc-800">
        ...
        <BarChartComp
          data={salaryPerformance}
          dataKeys={["salary", "performance"]}
          dualAxis={true}
